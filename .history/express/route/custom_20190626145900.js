...
router.post('/', (req, res) => {
    ...
    false.rename(oldPath, newPath, err => {
        if (err) {
            ...
            if (req.body.mod_id) {
                db.query(`UPDATE custom_table SET title='${req.body.title}',description='${req.body.description}',title='${req.body.src}' WHERE ID=${req.body.mod_id}`, (err, data) => {
                    if (err) {
                        console.error(err)
                        res.status(500).send('database error').end()
                    } else {
                ...
                db.query(`INSERT INTO custom_table (title, description, hrsrcef) VALUES('${title}', '${description}', '${newFileName}')`, (err, data) => {
                    if (err) {
                        console.error(err)
                        res.status(500).send('database error').end()
                    } else {
